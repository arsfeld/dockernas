...
(function () {
    ...
    module.exports = {
         ...
         * @returns {Promise}
         */
        login: function (repository) {
            return new Promise(function (resolve, reject) {
                //if (!username || !password) {
                //    return reject(new Error('Both username and password must be passed to this function!'));
                //}

                let url = `https://auth.docker.io/token?service=registry.docker.io&scope=repository:${repository}:pull"`
                return request({url, json: true}, (err, response, body) => {
                    loggedInToken = body['token']
                    resolve();
                }, () => {
                    reject();
                })
         ...
         * @returns {Promise}
         */
        tagsList: function (username, name, options) {
            return new Promise(function (resolve, reject) {
                if (!username || typeof username !== 'string') {
                    return reject(new Error('Username must be provided!'));
                }
                ...
                this.makeGetRequest(`repositories/${username}/${name}/tags?page_size=${options.perPage || 100}&page=${options.page || 1}`, 'results').then(resolve).catch(reject);
            }.bind(this));
        },
        /**
         * Get the history of all the builds done for a given repository.
         ...
         * @returns {Promise}
         */
        getManifest: function (username, name, manifest, options) {
            return new Promise(function (resolve, reject) {
                if (!username || typeof username !== 'string') {
                    return reject(new Error('Username must be provided!'));
                }
                ...
                this.makeGetRequest(`${username}/${name}/manifests/${manifest}`).then(resolve).catch(reject);
            }.bind(this));
        },
        /**
         * Get the history of all the builds done for a given repository.
        ...
        makeRequestParams(method, path, data) {
            ...
            let url = '';
                url = `https://registry.hub.docker.com/v${apiVersion}/${path}`;
